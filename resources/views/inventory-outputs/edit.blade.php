...
@section('content')
<div class="container">
    <h1>Editar Salida de Inventario</h1>
    ...
    <form action="{{ route('inventory-outputs.update', $output->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="item_codigo" id="item_codigo" class="form-control" required>
                ...
                @foreach ($items as $item)
                    <option value="{{ $item->codigo }}" {{ (old('item_codigo', $output->item_codigo) == $item->codigo) ? 'selected' : '' }}>
        ...
        <div class="mb-3">
            ...
            <select name="user_ci" id="user_ci" class="form-control" required>
                ...
                @foreach ($users as $user)
                    <option value="{{ $user->ci }}" {{ (old('user_ci', $output->user_ci) == $user->ci) ? 'selected' : '' }}>
                        {{ $user->nombre }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <textarea name="observacion" id="observacion" class="form-control" rows="3">{{ old('observacion', $output->observacion) }}</textarea>
        </div>
